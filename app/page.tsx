...
return (
            ...
            <div className="space-y-2">
              {quickActions.map((action, index) => (
                <button 
                  ...
                  className="w-full text-left p-3 text-sm text-gray-300 hover:bg-gray-800 border border-transparent rounded-lg transition-all duration-200 hover:shadow-sm"
                >
                  <div className="font-medium mb-1">{action.title}</div>
                  <div className="text-gray-400 text-xs line-clamp-2">{action.prompt}</div>
                </button>
              ))}
            </div>
          </div> */}
          ...
          {messages.length === 0 && (
            <div className="h-full flex items-center justify-center">
              <div className="text-center max-w-2xl">
                {/* <div className="w-24 h-24 bg-gradient-to-r from-green-500 to-blue-600 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-xl">
                ...
                {/* <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-xl mx-auto">
                  {quickActions.map((action, index) => (
                    <button
                      key={index}
                      onClick={() => handleQuickAction(action.prompt)}
                      className="bg-gray-800 p-4 rounded-xl border border-gray-700 shadow-sm hover:shadow-md transition-all duration-200 text-left group"
                    >
                      <div className="text-blue-500 font-semibold mb-2 group-hover:text-blue-400">{action.title}</div>
                      <div className="text-gray-400 text-sm line-clamp-2">{action.prompt}</div>
                    </button>
                  ))}
                </div> */}
              </div>
    ...
    <span>{messages.length} message{messages.length !== 1 ? 's' : ''}</span>
        ...
        {/* <div className="border-t border-gray-800 bg-black p-4 sm:p-6">
          <div className="max-w-6xl mx-auto">
            <div className="flex space-x-4">
              <div className="flex-1 relative">
                <textarea
                  ...
                  onChange={(e) => setInput(e.target.value)}
                  onKeyDown={handleKeyDown}
                  placeholder="Ask Llama 3.3 70B anything... (Press Enter to send, Shift+Enter for new line)"
                  className="w-full px-5 py-4 pr-12 border border-gray-700 rounded-2xl resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-gray-800 placeholder-gray-400 transition-all duration-200 text-[15px] leading-relaxed text-white"
                  rows={1}
                  disabled={loading}
                />
              ...
              <button
                onClick={sendMessage}
                disabled={!input.trim() || loading}
                className="px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl font-semibold hover:from-blue-700 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center space-x-3 shadow-lg hover:shadow-xl disabled:shadow-none"
              >
                {loading ? (
                  <>
                    <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
            ...
            <div className="flex items-center justify-between mt-3 px-1 text-gray-400 text-xs">
              <span>Llama 3.3 70B via Groq • May produce inaccurate information</span>
              <span>{messages.length} message{messages.length !== 1 ? 's' : ''}</span>
            </div>
          </div>
  ...
  //     {/* Header */}
  //     <div className="bg-black border-b border-slate-200 px-6 py-4 flex items-center justify-between shadow-sm">
  //       <div className="flex items-center space-x-4">
  //         <div className="w-10 h-10 bg-gradient-to-r from-green-500 to-blue-600 rounded-xl flex items-center justify-center shadow-lg">
  //           <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  //             <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
  ...
  //         </div>
  //         <div>
  //           <h1 className="text-xl font-bold text-slate-800">ChatBot</h1>
  //           <p className="text-slate-500 text-sm">Powered by Groq • Lightning fast AI</p>
  //         </div>
  //       </div>
  ...
  //         <button
  //           onClick={clearChat}
  //           className="flex items-center space-x-2 px-4 py-2 text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-lg transition-colors duration-200 border border-slate-200"
  //         >
  //           <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  //             <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
  //           </svg>
  ...
  //     <div className="flex-1 flex overflow-hidden">
  //       {/* Sidebar */}
  //       <div className="w-80 bg-black border-r border-slate-200 hidden lg:block p-6">
  //         <div className="space-y-6">
  //           <div>
  ...
  //                   key={index}
  //                   onClick={() => handleQuickAction(action.prompt)}
  //                   className="w-full text-left p-3 text-sm text-slate-700 hover:bg-blue-50 hover:border-blue-200 border border-transparent rounded-lg transition-all duration-200 hover:shadow-sm"
  //                 >
  //                   <div className="font-medium mb-1">{action.title}</div>
  //                   <div className="text-slate-500 text-xs line-clamp-2">{action.prompt}</div>
  //                 </button>
  //               ))}
  //             </div>
  //           </div>
  ...
  //           <div>
  //             <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-3">Model Info</h3>
  //             <div className="bg-slate-50 rounded-lg p-4 space-y-3">
  //               <div className="flex justify-between items-center">
  //                 <span className="text-sm text-slate-600">Model:</span>
  //                 <span className="text-sm font-medium text-slate-800">Llama 3.3 70B</span>
  //               </div>
  //               <div className="flex justify-between items-center">
  //                 <span className="text-sm text-slate-600">Provider:</span>
  //                 <span className="text-sm font-medium text-slate-800">Groq</span>
  //               </div>
  //               <div className="flex justify-between items-center">
  ...
  //                   </svg>
  //                 </div>
  //                 <h2 className="text-3xl font-bold text-slate-800 mb-4">Chat with Llama 3.3 70B</h2>
  ...
  //                       key={index}
  //                       onClick={() => handleQuickAction(action.prompt)}
  //                       className="bg-black p-4 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 text-left hover:border-blue-300 group"
  //                     >
  //                       <div className="text-blue-600 font-semibold mb-2 group-hover:text-blue-700">{action.title}</div>
  //                       <div className="text-sm text-slate-500 line-clamp-2">{action.prompt}</div>
  //                     </button>
  //                   ))}
  //                 </div>
  //               </div>
  ...
  //                       {msg.role === "user" ? "You" : "Llama 3.3 70B"}
  //                     </span>
  ...
  //                         : "bg-black text-slate-700 border border-slate-200 rounded-bl-md shadow-sm"
  //                     }`}
  //                   >
  //                     <div className="whitespace-pre-wrap leading-relaxed text-[15px]">{msg.content}</div>
  //                   </div>
  ...
  //                 <div className="flex-1">
  //                   <div className="flex items-center space-x-2 mb-1">
  //                     <span className="text-sm font-medium text-slate-700">Llama 3.3 70B</span>
  //                     <span className="text-xs text-slate-400">Thinking...</span>
  //                   </div>
  //                   <div className="bg-black border border-slate-200 rounded-2xl rounded-bl-md px-6 py-4 shadow-sm">
  //                     <div className="flex space-x-2 items-center">
  ...
  //                       <span className="text-sm text-slate-500 ml-2">Processing with Groq...</span>
  //                     </div>
  //                   </div>
  //                 </div>
  //               </div>
  ...
  //         {/* Input Area */}
  //         <div className="border-t border-slate-200 bg-black p-6">
  //           <div className="max-w-6xl mx-auto">
  //             <div className="flex space-x-4">
  //               <div className="flex-1 relative">
  //                 <textarea
  ...
  //                   onKeyDown={handleKeyDown}
  //                   placeholder="Ask Llama 3.3 70B anything... (Press Enter to send, Shift+Enter for new line)"
  //                   className="w-full px-5 py-4 pr-12 border border-slate-300 rounded-2xl resize-none focus:outline-none focus:ring-3 focus:ring-blue-500/20 focus:border-blue-500 bg-slate-50 placeholder-slate-400 transition-all duration-200 text-[15px] leading-relaxed"
  //                   rows={1}
  //                   disabled={loading}
  //                 />
  ...
  //                 onClick={sendMessage}
  //                 disabled={!input.trim() || loading}
  //                 className="px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-2xl font-semibold hover:from-blue-700 hover:to-indigo-700 focus:outline-none focus:ring-3 focus:ring-blue-500/30 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center space-x-3 shadow-lg hover:shadow-xl disabled:shadow-none"
  //               >
  //                 {loading ? (
  //                   <>
  //                     <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div>
  ...
  //                 Llama 3.3 70B via Groq • May produce inaccurate information
  //               </div>
  //               <div className="text-xs text-slate-400">
